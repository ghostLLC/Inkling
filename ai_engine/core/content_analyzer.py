...
class ContentAnalyzer:
    ...
    def analyze(self, text: str, cursor_pos: int | None = None) -> AnalysisResult:
        """分析文本内容"""
    ...
    def _detect_section(self, text: str, cursor_pos: int | None = None) -> tuple[str, float]:
        """检测当前写作段落和进度"""
    ...
    def _get_last_paragraph(self, text: str, cursor_pos: int | None = None) -> str:
        """获取最后一段或光标所在段落"""
    ...
    def _detect_stuck_type(self, last_paragraph: str, full_text: str) -> tuple[StuckType | None, float]:
        """基于最后一段内容检测卡壳类型"""
