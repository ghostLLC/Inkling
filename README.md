...
│   │       ├── base.py        # Handler 基类
│   │       ├── topic_analysis.py  # 审题立意
│   │       ├── stuck_rescue.py    # 卡壳救援
│   │       ├── ending_guide.py    # 结尾收束
│   │       └── complete.py        # 写作完成
...
│   ├── css/style.css
│   ├── js/app.js
│   └── static_legacy/        # 旧版 Demo（保留）
├── scripts/                   # 工具脚本 & Legacy
│   └── api_server.py          # 旧版 stdlib 服务器（已弃用）
├── tests/
│   ├── test_flows.py          # 18 单元测试
│   ├── integration/           # Use Case 集成测试
│   └── e2e/                   # 端到端测试
├── docs/
