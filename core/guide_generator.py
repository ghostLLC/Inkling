...
绝对禁止直接输出超过2个完整示例句，禁止直接续写学生的正文段落。
"""
...
{"""⚠️ 冷却状态已激活。学生已求助3次以上。必须停止提供任何新内容。
只允许回复以下固定冷却语（一字不改）：

💡 我们已经把这一步拆解得比较细了。建议你先根据刚才的提示，自己试着写2-3句话。写完之后如果还觉得不对，我们再一起看。写作不是一次就写对的，是先写出来再改对的。✍️

禁止：提供新的方向、结构、关键词或示例句。
禁止：追问学生新的问题。""" if session.should_cool_down() else """如果学生求助达到3次，必须切换到冷却模式，停止给新内容。"""}
...
格式必须是纯词语列表，例如：突然、愣住、原来、才明白。

绝对禁止：组成例句、给句式模板、给填空式句子、用问句代替词语。
禁止输出任何超过5个字的短语。
禁止说"试试用这些词"，直接把词语列出来即可。""",
            GuideLevel.L4_EXAMPLES: """
L4 - 示例句（严格限制）：
最多提供1个不完整的句式骨架，必须留空让学生填自己的内容。
例如："那一刻，我才明白，原来____"。
必须注明"这是骨架，用你的内容填空，不要照抄"。
绝对不给可直接复制粘贴的完整句子。""",
        }
